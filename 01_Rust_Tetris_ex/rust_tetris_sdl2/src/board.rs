use crate::{bdimentions, gameend, level, player};
...
impl Board {
    pub fn new() -> Board {
        ...
        for j in 0..20 {
            ...
            for i in 0..10 {
                let positioned_retangle: sdl2::rect::Rect = sdl2::rect::Rect::new(
                    2 + dimentions.left + (i * dimentions.unit_size),
                    dimentions.top + 2 + (dimentions.unit_size * j),
                    (dimentions.unit_size - 1) as u32,
                    (dimentions.unit_size - 1) as u32,
                );
                let p: Piece = Piece {
                    rect: positioned_retangle,
                    color: clr,
                    occupied: false,
                };
                row.push(p);
        ...
        for j in 0..4 {
            let mut row: Vec<Piece> = Vec::new();
            for i in 0..4 {
                let positioned_retangle_right: sdl2::rect::Rect = sdl2::rect::Rect::new(
                    2 + dimentions.right + dimentions.unit_size + (i * dimentions.unit_size),
                    dimentions.top + 2 + (dimentions.unit_size * j),
                    (dimentions.unit_size - 1) as u32,
                    (dimentions.unit_size - 1) as u32,
                );
                row.push(Piece {
                    rect: positioned_retangle_right,
                    color: light_black,
                    occupied: false,
                })
            }
        ...
        Board {
            bmatrix: board_pieces,
            preview_matrix: preview_board_pieces,
            players: player_list,
            level_text: level::Level::new(),
            rows_cleared: 0,
            score: 0,
            end: false,
        }
    ...
    pub fn draw_a_player(&mut self) {
        ...
        if first {
            // Test if fits, if it doesn't need to end game
